...
variable SPACK_CONCRETE_ENVIRONMENT_PATH.""")
    ...
    # Handle preinstall and postprocessing steps of a ci (re)build
    rebuild = subparsers.add_parser('rebuild', help=ci_rebuild.__doc__)
    rebuild.set_defaults(func=ci_rebuild)
    task_group = rebuild.add_mutually_exclusive_group()
    task_group.add_argument(
        '--prepare', action='store_true', dest='pre_process', default=True,
        help="Run ci rebuild preparation tasks")
    task_group.add_argument(
        '--postprocess', action='store_false', dest='pre_process', default=True,
        help="Run ci rebuild postprocessing tasks")
